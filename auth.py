"""智慧树 Cookie 持久化与微信扫码登录模块（Playwright async / Chromium）。"""
...
async def _click_wechat_login(page) -> bool:
    selectors = [
...
async def _capture_qrcode(page, qrcode_path: str) -> None:
    try:
        ...
        if el:
            ...
            await el.screenshot(path=qrcode_path)
            return
    except Exception:
        pass
    await page.screenshot(path=qrcode_path)


async def _build_session_from_context(context) -> Optional[requests.Session]:
    try:
...
async def _try_valid_session(context, page, cookie_path: str,
                             app_token: str, uid: str) -> Optional[requests.Session]:
    session = await _build_session_from_context(context)
    if not session:
        return None
...
async def _try_fallback_session(context, page, cookie_path: str,
                                app_token: str, uid: str) -> Optional[requests.Session]:
    session = await _build_session_from_context(context)
    if not session:
        return None
...
async def _try_any_session(context, page, cookie_path: str,
                           app_token: str, uid: str) -> Optional[requests.Session]:
    session = await _try_valid_session(context, page, cookie_path, app_token, uid)
    return session or await _try_fallback_session(context, page, cookie_path, app_token, uid)


async def _login_progressed(page) -> bool:
    try:
...
async def _goto_online_home(page) -> None:
    selectors = [
    ...
    for sel in selectors:
        try:
            ...
            continue
    try:
        await page.goto(config.ONLINE_HOME_URL)
        await asyncio.sleep(2)
    except Exception:
        pass


async def _sync_domains(page) -> bool:
    try:
        await _goto_online_home(page)
        await page.goto(config.ONLINE_HOME_URL)
        await asyncio.sleep(2)
        await page.goto("https://www.zhihuishu.com")
        await asyncio.sleep(1)
        await page.goto("https://hike-examstu.zhihuishu.com")
        await asyncio.sleep(1)
        await page.goto("https://onlineservice.zhihuishu.com")
        await asyncio.sleep(2)
        return True
    except Exception:
        return False
...
async def _browser_verify(page) -> bool:
    try:
        ts = int(time.time() * 1000)
        await page.goto(f"{config.VERIFY_URL}?dateFormate={ts}")
        await asyncio.sleep(1)
...
async def _debug_snapshot(page, label: str, app_token: str, uid: str) -> None:
    try:
...
async def _debug_cookie_state(context, page, label: str, app_token: str, uid: str) -> None:
    try:
        raw = await context.cookies()
...
async def _handle_progress(context, page, cookie_path, app_token, uid,
                           synced: bool, snapshotted: bool):
    if not synced:
        synced = await _sync_domains(page) or synced
    if not snapshotted:
        await _debug_snapshot(page, "progress", app_token, uid)
        snapshotted = True
    await _browser_verify(page)
    session = await _try_any_session(context, page, cookie_path, app_token, uid)
    return session, synced, snapshotted


async def _wait_for_login(context, page, cookie_path: str,
    ...
    while time.time() - start < timeout:
        progressed = await _login_progressed(page)
        if progressed or (time.time() - last_sync_at >= 8):
            session, synced, snapshotted = await _handle_progress(
    ...
    await _debug_snapshot(page, "timeout", app_token, uid)
    await _debug_cookie_state(context, page, "timeout", app_token, uid)
...
async def prepare_qrcode(headless: bool = True, data_dir: str = "data"):
    ...
    try:
        ...
        if not await _click_wechat_login(page):
            await browser.close()
            await p.stop()
        ...
        await _capture_qrcode(page, qrcode_path)
...
async def complete_login(playwright_objects, cookie_path: str, timeout: int = 120):
    """等待用户扫码，收集 Cookie 并保存。返回 requests.Session 或 None。"""
    p, browser, context, page = playwright_objects
    try:
        session = await _wait_for_login(context, page, cookie_path, "", "", timeout)
